...
//  i41CV
//
//  Created by Miroslav Perovic on 8/16/14.
...
class CStruct : NSObject {
	...
	let ppointer: Int = sizeof(UnsafePointer)
	let bytesForValue = [
		Ops.SkipByte:		1,
		Ops.PackChar:		1,
		Ops.PackInt8:		1,
	...
	func pack(values: [AnyObject], error: NSErrorPointer) -> NSData? {
		...
		for op in self.opStream {
			...
			default:
				...
				case .PackPointer:
					if let value = rawValue as? UInt {
						padAlignment(sizeof(UnsafePointer))
						bytes.extend(value.splitBytes(endianness, size: sizeof(UnsafePointer)))
					} else {
						return failure("cannot convert argument to UInt")
					}
